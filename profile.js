  ...
  fetch("https://hakims-webshop.herokuapp.com/user/register", {
    ...
    .then(function (response) {
      if (response == "Error") {
        ...
        alert(
          response.firstName +
            " har registrerats som ny användare. \nVänligen logga in för att lägga en beställning."
        );
        location.reload();
      }
...
function deleteUser() {
  ...
  fetch(`https://hakims-webshop.herokuapp.com/user/delete/${deleteUser.username}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer " + token,
